    ...
    public static class UserFunctions
    {
        ...
        internal static void ReadList()
        {
            try
            {
                ...
                foreach (var article in Data.Articles)
                    Utils.PrintRow(ConsoleColor.White, article.Id.ToString(), article.Name,
                        article.Price.ToString(CultureInfo.InvariantCulture), article.Count.ToString());
                Utils.PrintLine();
                var endTimeReadPrint = DateTime.Now.Ticks;
                Logger.AddLine(
                    $"reading and printing {Data.Articles.Count} took: Action took: {endTimeReadPrint / TimeSpan.TicksPerMillisecond - startTimeReadPrint / TimeSpan.TicksPerMillisecond} milliseconds ({endTimeReadPrint - startTimeReadPrint} ticks)");
        ...
        internal static void SearchList()
        {
            ...
            try
            {
                ...
                Logger.AddLine(
                    $"reading and printing {Data.Articles.Count} took: Action took: {endTime / TimeSpan.TicksPerMillisecond - startTime / TimeSpan.TicksPerMillisecond} milliseconds ({endTime - startTime} ticks)");
                Console.Write("Press any key to return to the menu...");
                Console.ReadKey();
            }
            catch (Exception e)
            ...
            try
            {
                ...
                if (input == "1" || input == "id")
                {
                    ...
                    foreach (var art in artList)
                        Utils.PrintRow(ConsoleColor.White, art.Id.ToString(), art.Name,
                            art.Price.ToString(CultureInfo.InvariantCulture), art.Count.ToString());
                    Utils.PrintLine();
                }
